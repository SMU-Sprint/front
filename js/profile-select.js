(() => {
  ...
  triggers.forEach(trigger => {
    ...
    trigger.dataset.placeholder ||= multiple ? (panel.classList.contains('day-panel') ? '요일 선택' : '시간 선택') : '운동 지역 선택';
    ...
    panel.querySelectorAll('button').forEach(button => {
      ...
      button.addEventListener('click', () => {
        if (!multiple) panel.querySelectorAll('button').forEach(b => b.classList.remove('selected'));
        button.classList.toggle('selected', !multiple || !button.classList.contains('selected'));
        const selected = [...panel.querySelectorAll('.selected')].map(b => b.dataset.value || b.textContent.trim());
        panel.querySelectorAll('button').forEach(b => b.setAttribute('aria-pressed', String(b.classList.contains('selected'))));
        trigger.dataset.values = JSON.stringify(selected);
        trigger.textContent = selected.length ? selected.join(', ') : trigger.dataset.placeholder;
        trigger.classList.toggle('active', selected.length > 0);
        trigger.dispatchEvent(new Event('change', { bubbles: true }));
        if (!multiple) { close(); trigger.focus(); }
      });
    });
    trigger.setAttribute('aria-expanded', 'false');
  ...
  document.addEventListener('click', e => { if (!e.target.closest('.select-field')) close(); });
