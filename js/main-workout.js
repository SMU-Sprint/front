(() => {
  ...
  function renderSession() {
    ...
    document.getElementById("session-sport").textContent =
      session?.sportName || "운동 대기";
    document.getElementById("session-location").textContent =
      session?.locationName || "장소를 선택해 시작하세요";
    document.getElementById("session-start").textContent = time(
      session?.startedAt,
    );
    document.getElementById("session-start-date").textContent = session
      ? dateKey(session.startedAt).replaceAll("-", "/")
      : "";
    document.getElementById("session-status").textContent = activeSession
      ? "운동 중"
      : lastSession
        ? "운동 종료"
        : "";
  ...
  function renderCalendar() {
    WorkoutCalendar.render(totals, selectedDate, selectDate);
  }
  ...
  async function selectDate(date) {
    ...
    try {
      ...
      if (token !== recordVersion) return;
      list.textContent = "기록을 불러오지 못했습니다. 새로고침을 눌러 주세요.";
  ...
  async function refresh() {
    ...
    try {
      const data = await service.overview(
        dateKey(new Date(now.getFullYear(), now.getMonth() - 2, 1)),
        dateKey(new Date(now.getFullYear(), now.getMonth() + 1, 0)),
      );
      ...
      if (token === version) message(error.message, true);
    }
  }
    ...
    ++version;
    renderSession();
    message("운동 기록을 저장하는 중입니다…");
    try {
      lastSession = await service.stop(activeSession.id);
      activeSession = null;
      selectedDate = dateKey(lastSession.endedAt);
      renderSession();
      message(
        service.demo
          ? "운동 기록을 이 브라우저에 저장했습니다."
          : "운동 기록을 저장했습니다.",
      );
      await refresh();
    } catch (error) {
      message(`${error.message} 종료하기를 다시 눌러 주세요.`, true);
    } finally {
      busy = false;
      renderSession();
    }
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === SprintWorkout.STORAGE_KEY && !busy) refresh();
  });
